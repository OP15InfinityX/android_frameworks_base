...
public class DuressPasswordHelper {
    ...
    protected void onVerifyCredentialResult(@Nullable VerifyCredentialResponse res, @Nullable LockscreenCredential credential) {
        ...
        // AOSP zeroizes credentials received through Binder as soon as credential verification
        // returns. Keep a private copy for this asynchronous comparison and wipe it afterwards.
        ...
        backgroundThread.getThreadHandler().post(() -> {
            try (credentialCopy) {
                if (isDuressCredential(credentialCopy)) {
    ...
    private void checkOwnerCredential(LockscreenCredential ownerCredential) {
        ...
        if (lockSettingsService.getCredentialType(userId) == CREDENTIAL_TYPE_NONE) {
            ...
            // GrapheneOS passes LockDomain.Primary here. AOSP/Infinity-X does not have
            // GrapheneOS's separate secondary lock domain, so its regular checkCredential()
            // overload is the equivalent primary-credential check.
            VerifyCredentialResponse response =
                    lockSettingsService.checkCredential(ownerCredential, userId, null);
